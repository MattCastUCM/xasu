...
namespace Xasu.Util
{
    internal static class WebGLUtility
    {
        ...
        {
            return PersistentPrefs.HasKey(UNITY_TRACKER_WEBGL_LISTENING) && PersistentPrefs.GetInt(UNITY_TRACKER_WEBGL_LISTENING) == 1;
        }

        public static void SetWebGLListening(bool value)
        {
            PersistentPrefs.SetInt(UNITY_TRACKER_WEBGL_LISTENING, value ? 1 : 0);
            PersistentPrefs.Save();
        }

        public static TrackerConfig GetUrlTrackerConfig()
        {
            XasuTracker.Log("Parsing tracker configuration from URL("+GetCompleteUrl()+") parameters...");
            ...
            {
                XasuTracker.Log("SSO configuration detected in URL parameters. Setting up OAuth2 authentication.\n" + ssoTokenEndpoint);
            ...
            {
                XasuTracker.Log(username + " detected in URL parameters. Setting up basic authentication with provided username and password.");
            ...
            if (!string.IsNullOrEmpty(resultUri))
            {
                ...
                {
                    XasuTracker.Log("Simva configuration detected in URL parameters. Setting up Simva behavior as true.");
                    trackerConfig.Simva = true;
                }
                XasuTracker.Log("Setting result endpoint from URL parameters: " + resultUri);
                trackerConfig.Online = true;
                trackerConfig.LRSEndpoint = resultUri;
            }
            if (!string.IsNullOrEmpty(backupUri))
            {
                XasuTracker.Log("Setting backup endpoint from URL parameters: " + backupUri + " with type " + backupType);
            ...
            {
                XasuTracker.Log("Tracker configuration from URL is invalid: No result_uri or backup_uri provided.");
                return null;
            }
