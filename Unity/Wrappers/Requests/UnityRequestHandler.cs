...
namespace Xasu.Requests
{
    internal class UnityRequestHandler : HttpRequestHandler
    {
        ...
        public UnityRequestHandler(bool sendRequestsInBackground = false)
        {
            _sendRequestsInBackground = sendRequestsInBackground;
        }

        public override async Task<HttpResponse> SendRequest(HttpRequest myRequest, IProgress<float> progress = null)
        {
        ...
        {
            if (_sendRequestsInBackground)
            {
                Application.runInBackground = true;
            }
            
            UnityWebRequestAsyncOperation asyncRequest = webRequest.SendWebRequest();
            ...
            if (_sendRequestsInBackground && !string.IsNullOrEmpty(backgroundError))
            {
        ...
        protected override bool IsSimvaStatements(HttpRequest myRequest)
        {
            // Simva Special cases
            if (XasuTracker.TrackerConfig != null && XasuTracker.TrackerConfig.Simva)
            {
                ...
                return base.IsSimvaStatements(myRequest);
            }
            return false;
        }
    }
